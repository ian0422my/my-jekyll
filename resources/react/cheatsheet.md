...
| js           | async getMovies() { const {data:m} = await http.getMovies(); return m;}              | fetch data and return only when request is fulfilled (promised); code hang if no response |
| js           | getMovies = async() => { const {data:m} = await http.getMovies(); return m;}         | fetch data and return only when request is fulfilled (promised)                           |
...
| react,rrd    | \<Route path="/posts/:year?" component={Posts} />; year=this.props.match.params.year | routing; path with optional parameters                                                    |
