...
* ~~docker engine spins off a linux container to run your process~~
  docker engine does not do virtualization
    * you will actually notice that what ever process runs in your containers appears in your host
...
* network
  * containers by default can see each other since all of them has the same ip mask (172.17.0.4/24)
* configuration file
...
ps -ef|grep dockerd // make sure -H tcp://0.0.0.0:2375 si there
```
