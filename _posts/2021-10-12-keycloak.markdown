...
* login to gitlab by visiting <http://localhost:8080/>
* login using "root" and getting the password by running command below in vagrant
...
* stop by running command below

```sh
sudo docker stop gitlab1
```

Login to Keycloak user and password is `admin:Pa55w0rd`

##### Trust keycloak self signed certificate (not needed if you are using non-https)
...
* configure logout url (workaround from <https://gitlab.com/gitlab-org/gitlab/-/issues/31203>)
  * menu > admin > general > sign-in restrictions > Sign-out page URL > http://keycloak_keycloak_1:8080/auth/realms/test/protocol/openid-connect/logout?redirect_uri=http://gitlab1:9080
...
* login through gitlab
  ...
  * login to keycloak using
    * keycloakuser1/Pa55w0rd
      * user will be automatically created into gitlab
  * redirect to gitlab homepage
* logout
