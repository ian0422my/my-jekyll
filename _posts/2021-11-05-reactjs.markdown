...
| js     | let errors = { errors: errors \|\| {} }                                              | return errors if is not null(or return empty object)                                 |
| js     | const counters = [...this.state.counters, {}]                                        | clone object array with additional object                                            |
| js     | totalCounters={this.state.counters.filter(c=>c.value>0).length}/>                    | copy with filtering condition                                                        |
...
| joi    | schema = {uname: Joi.required().label("User Name")};Joi.validate({uname:""}, schema) | define Joi schema,validate object and return validation message with friendly name   |
...
| react  | \<Route render={(props)=>\<Product sortBy="newest" {...props}/>}/>                   | render component based on path with custom+standard props                            |
