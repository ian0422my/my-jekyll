...
* Router
  ...
  * parameters
    * optional parameters is better as queyr parameter than url parameters
...
| ccc + tab                                                                         | create class component with constructor                                     |
...
| const {handleReset: customVarName} = this.props; customVarName();                 | destructing argument with custom variable name                              |
| onClick={() => handleReset(counters)}                                             | pass reference to a method with arg                                         |
| state={m1: name => <h1>welcome {name}</h1>}; <Profile m1={this.state.m1}/>        | pass reference to a method with arg as object member                        |
...
| parent - <Counter fn1={fn1}>; child - this.props.fn1()                            | passing method(no arg) reference from parent to child                       |
| parent - <Counter fn1={fn1}>; child - () => this.props.fn1(param1,param2)         | passing method(with arg) reference from parent to child                     |
| let counters = this.state.counters.filter((counter)=>counter.id!==target.id));    | filter                                                                      |
| this.state.counters.map((counter)=>console.log(counter.id));                      | iterate                                                                     |
| table.table>thead>tr>td*4>h1+span.m-2[style="cursor:pointer"] + tab               | generate table with thead,tr,4 Td, h1 and span(same level) with style; zen  |
| this.setState({movies}); // rather than {movie:movie}.                            | ecma6; use when key=value;shorter and cleaner code                          |
| let pages = lodash.range(min, max);                                               | create array with min, max size                                             |
| _(items).slice(startIndex).take(pageSize).value()                                 | cut a chuck sizes(pageSize) of items from start(index)                      |
| Pagination.propTypes = {itemCount: PropTypes.number.isRequired}                   | enforce type - number and required - for a class member                     |
| const a1 = [1,2,3]; const a2 = [4,5,6];const combined = [...a1, 'a', ...a2, 'b']; | combine array and new item                                                  |
| <span className={{m-2 col-2}}/><span className={{col}}>                           | span in 1st column(grid system has 12 columns); 2nd span takes all column   |
| item={id: "1", name:"ian"}; key="id";item[key] == 1;//true                        | item[key] access value of items dynamically (i.e. no hardcode like item.id) |
| MyComponent.defaultProps = { customPropVal: "default value"}                      | set default value for props member                                          |
| _.get(object, path)                                                               | get object member by path                                                   |
| <Router path="/path" component="Products"/>                                       | render component based on path with standard props                          |
| <Router render={(props)=><Product sortBy="newest" {...props}/>}/>                 | render component based on path with custom+standard props                   |
| <Link to="/path"/>                                                                | change url without reloading bundle.js                                      |
| <NavLink to="/path">                                                              | change url; highlight automatically once clicked                            |
| <Switch>                                                                          | render component based on path. If match, stop processing the rest          |
| <Route path="/posts/:year?" component={Posts} />; year=q.get                      | path with optional parameters                                               |
| query param > ?year > let {year} = queryString.parse(this.props.location.search); |                                                                             |
| router param > /:year/ > let year = this.props.match.params.year                  |                                                                             |
| change url > this.props.history.push(url) or replace(url)                         |                                                                             |
| npm i query-string@6.1.0; queryString.parse(location.search);                     | parse queryString as object                                                 |

## Tutorial - ES6 Refresher
...
## Tutorial - Pagination, Filtering and Sorting
...
* use componentDidMount follow by setState
  ...
  UserList() {
    ...
    fetch('https://randomuser.me/api/') 
      ...
      .then(function(myJson) { 
        that.setState({person: myJson.results}); // for example
      });
  }

  render() { 
    return <div>Welcome {this.state.person.map(p=>p.name.first + " " + p.name.last)}</div>
  }
}
...
fetch('https://randomuser.me/api/', 
  {
    ...
    body: {
      JSON.stringify("first_name": this.firstName.value)
    }
  })
.then(function(response) { return response.json(); }) 
.then(function(myJson) { 
